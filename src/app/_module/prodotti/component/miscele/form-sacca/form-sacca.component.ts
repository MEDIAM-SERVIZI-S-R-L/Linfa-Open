...
@Component({
  selector: 'app-form-sacca',
  templateUrl: './form-sacca.component.html',
  styleUrls: ['./form-sacca.component.scss']
})
export class FormSaccaComponent implements OnInit {

  defaultBase64String: string;
  form: FormGroup;

  listaVitamine: any;
  listaOligoelementi: any;
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this.service.getCallRequest({ action: 'getListeAggiunte', param: TipoAggiunta.Vitamine }).then(data => this.listaVitamine = data);
    await this.service.getCallRequest({ action: 'getListeAggiunte', param: TipoAggiunta.Oligoelementi }).then(data => this.listaOligoelementi = data);
    await this.service.postCallRequest({ action: 'getKcalBaseSacche', param: {} }).then(data => this.listaKcalBaseProprieta = data);
  ...
  showLabel(id: number): string {
    if (!this.appGen.isNullOrUndefined(this.listaKcalBaseProprieta)) {
      const elemento = this.appGen.getElementByFilter(id, this.listaKcalBaseProprieta)[0];
      if (this.appGen.isNullOrUndefined(elemento)) {
        return;
      }

      if (elemento.abilitato) {
        return elemento.kcal + ' kcal / ' + elemento.quantita + ' ' + (this.appGen.isNullOrUndefined(elemento.unitaMisura) ? '-' : elemento.unitaMisura)
      }
    }
    return ''
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    await this.service.postCallRequest(objRequest).then(async (data): Promise<void> => {
      if (this.fromModal) {

        //restituisco l'oggetto intero in modo da inserirlo nella tabella
        this.modalSaccaEmit.emit(data);

      } else {
        this.router.navigate(['/app/prodotti/miscele']);
      }
    });
  ...
  async anteprimaSacche(): Promise<void> {
    ...
    const obj = {
      pdfFile: this.report.createDocumentoSaccaPersonalizzata(this.prodotto),
      isReferto: false,
      title: 'Anteprima sacca personalizzata',
      nomeDocumento: this.form.get('prodotto').value
    }
